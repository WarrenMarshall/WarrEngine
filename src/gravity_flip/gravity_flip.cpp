...
gravity_flip_game::gravity_flip_game( std::string name )
	: game( name )
{
}
...
time_ms gravity_flip_game::get_ship_powerup_timer_duration()
{
...
void gravity_flip_game::level_up_player_ship()
{
	ship_power_level++;
	ship_power_level = glm::min<int>( ship_power_level, max_ship_power_level - 1 );
}

bool gravity_flip_game::ship_is_at_max_level()
{
	return true;
