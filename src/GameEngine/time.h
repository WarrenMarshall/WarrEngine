...
struct w_time
{
	...
	static const int FTS_desired_frames_per_second;
	// how large a time step in fixed time is - computed automatically
	static const int FTS_step_value_ms;

	// a max value that prevents us from time stepping too far whenever
	// something happens that would cause a huge step in time that can
	// mess things up - like a window drag or a system stall
	static const float delta_time_ms_max;
