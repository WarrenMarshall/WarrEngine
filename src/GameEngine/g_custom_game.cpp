...
g_custom_game::g_custom_game()
{
	...
	while( !tok.is_eos() )
	{
		...
		if( line.length() > 0 )
		{
			...
			tile_name = tile_name.substr( 1, tile_name.length() - 2 );	// remove quotation marks
			float tile_xpos = w_parser::parse_float_value( line_tok.get_next_token() );
			float tile_ypos = w_parser::parse_float_value( line_tok.get_next_token() );
			bool tile_show_in_browser = w_parser::parse_bool_value( line_tok.get_next_token() );

			tile_masters[ tile_id ].id = tile_id;
			tile_masters[ tile_id ].name = tile_name;
			tile_masters[ tile_id ].show_in_browser = tile_show_in_browser;
			tile_masters[ tile_id ].img = std::make_unique<w_image>( "tex_ultima_atlas", w_rect( tile_xpos, tile_ypos, TILE_SZ, TILE_SZ ) );
		}
	...
	for( int x = 0 ; x < ROOM_W * ROOM_H ; ++x )
		test_room.tiles[ x ] = engine->random->geti_range( 3, 4 );
}
