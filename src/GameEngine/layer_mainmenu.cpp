...
{
	//engine->render->draw_texture( grad_teal_blue, w_rect( 0.0f, 0.0f, v_window_w, v_window_h), 0.0f );
	//engine->render->draw_sprite( logo->get_texture(), w_vec2( engine->render->rcv.w / 2, 65 - ( tween_logo->get_fval() * 10.f ) ), W_COLOR_WHITE, 0.f, 0.75f );
	w_layer::draw();
	//engine->render->draw_sprite( germ->get_texture(), w_vec2( 100, 144 ), W_COLOR_WHITE, 0.f, 0.75f );
}
