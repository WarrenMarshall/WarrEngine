...
struct w_physics_query final
{
	...
	_NODISCARD static bool point_check_simple( const w_vec2& pos, int layer_mask );
	_NODISCARD static bool point_check_simple( const w_vec2& pos, int layer_mask, w_query_first* hit_result );
	_NODISCARD static bool point_check_all( const w_vec2& pos, int layer_mask, w_query_all* hit_result );
};
