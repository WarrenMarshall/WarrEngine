...
void layer_game::update()
{
	...
	auto player_h = find_entity( w_hash( "player_paddle_h" ) );
	assert( player_h );

	auto player_v = find_entity( w_hash( "player_paddle_v" ) );
	assert( player_v );
