...
void e_paddle::phys_begin_contact( w_pending_collision& coll, w_entity* other )
{
	...
	if( other->tag == "ball" )
	{
		...
		w_vec2 push_away_dir = ( other->get_transform()->pos - get_transform()->pos ).normalize();
...
void e_ball::phys_begin_contact( w_pending_collision& coll, w_entity* other )
{
	...
	if( other->tag == "ball" )
	{
		...
		phys_get_primary_body()->body->ApplyForceToCenter( ( ( get_transform()->pos - mid_point ).normalize() * e_ball::speed ).to_b2d().as_b2Vec2(), true );
		other->phys_get_primary_body()->body->ApplyForceToCenter( ( ( other->get_transform()->pos - mid_point ) * e_ball::speed ).normalize().to_b2d().as_b2Vec2(), true );
	}
