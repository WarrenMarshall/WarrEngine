...
void Particle_Emitter::set_params( const Emitter_Parameters_Asset* params )
{
	...
	{
		max_particles_alive = (i32)( params->s_max_alive );
	}
	else
	{
		max_particles_alive = (i32)( params->s_spawn_rate * ( params->r_lifespan.end / 1000.f ) );
	}

	// if "s_max_alive" has been set in the params, that overrides any calculated value from above
	max_particles_alive = ( params->s_max_alive > 0.f ) ? (i32)params->s_max_alive : max_particles_alive;
...
void Particle_Emitter::update()
{
	...
	i32 particles_to_spawn = 0;

	if( params->is_one_shot )
	{
		particles_to_spawn_accum = 0.f;
		particles_to_spawn = (i32)( params->s_max_alive );
	}
	else
	{
		// strip off the fractional part of the accum to get the number to spawn
		particles_to_spawn = (i32)( glm::trunc( particles_to_spawn_accum ) );
	}
...
{
	f32 max_life_span = params->r_lifespan.end;
