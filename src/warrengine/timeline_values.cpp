...
float_t Timeline_Values::get_float_value( float_t pct_on_timeline )
{
	...
	float_t pct_range = key_frames[ kf_max ].pct_marker - key_frames[ kf_min ].pct_marker;

	// the pct we are at within the min/max key frames
	float_t pct_within = ( pct_on_timeline - key_frames[ kf_min ].pct_marker ) / pct_range;

	// compute the value represented by that pct_within within the min/max key frames

	float_t min_value = key_frames[ kf_min ].float_value;
	float_t max_value = key_frames[ kf_max ].float_value;
