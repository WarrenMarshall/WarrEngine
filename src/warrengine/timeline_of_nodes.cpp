...
Timeline_Of_Nodes::Key_Frame::Key_Frame( e_tnkf_type type, bool should_restore_state, f32 pct_marker, time_ms duration )
...
void Timeline_Of_Nodes::Key_Frame::update()
{
	...
	auto pct = glm::clamp( ( time_now - started ) / (f32)duration, 0.f, 1.f );
...
void Timeline_Of_Nodes::update()
{
	...
	f32 pct_on_timeline = glm::clamp( ( g_engine->clock.now() - start ) / (f32)duration, 0.f, 1.f );
...
void Timeline_Of_Nodes::add_kf_shake_angle( bool should_restore_state, f32 pct_marker, time_ms duration, Transform* tform, f32 strength )
{
...
void Timeline_Of_Nodes::add_kf_pp_color_overlay( bool should_restore_state, f32 pct_marker, time_ms duration, Color color )
{
...
void Timeline_Of_Nodes::add_kf_pp_pixelate( bool should_restore_state, f32 pct_marker, time_ms duration, f32 pixelate_factor )
{
...
void Timeline_Of_Nodes::add_kf_play_sound( bool should_restore_state, f32 pct_marker, const Sound_Asset* snd )
{
...
void Timeline_Of_Nodes::add_kf_scene_push_under( bool should_restore_state, f32 pct_marker, std::unique_ptr<Scene> scene_to_push )
{
...
void Timeline_Of_Nodes::add_kf_scene_pop_under( bool should_restore_state, f32 pct_marker )
{
	key_frames.emplace_back( e_tnkf_type::scene_pop_under, should_restore_state, pct_marker, 0 );
}

void Timeline_Of_Nodes::add_kf_reset_scene_stack_to_main_menu( bool should_restore_state, f32 pct_marker )
{
