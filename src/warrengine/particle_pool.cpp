...
void Particle_Pool::draw()
{
	for( const auto& particle : objects )
	{
		...
		// lifetime
		f32 interp_life_span = calc_interpolated_per_sec_value( particle.life_span, -fixed_time_step::ms_per_step );
		...
		// render state
		Render::state->angle = calc_interpolated_per_sec_value( particle.spin, particle.spin_per_sec );
		...
		// position
		Vec2 interp_pos = {
			calc_interpolated_per_sec_value( particle.pos.x, ( particle.v_dir.x * particle.velocity_per_sec ) ),
			calc_interpolated_per_sec_value( particle.pos.y, ( particle.v_dir.y * particle.velocity_per_sec ) )
		};
