...
bool Gradient_Source_Asset::create()
{
	...
	w = (float)( ( alignment == e_align::horizontal ) ? ( colors.size() / 4 ) : 1 );
	h = (float)( ( alignment == e_align::vertical ) ? ( colors.size() / 4 ) : 1 );
	...
	glTextureParameteri( gl_id, GL_TEXTURE_WRAP_S, ( alignment == e_align::vertical ) ? GL_REPEAT : GL_CLAMP_TO_EDGE );
	glTextureParameteri( gl_id, GL_TEXTURE_WRAP_T, ( alignment == e_align::horizontal ) ? GL_REPEAT : GL_CLAMP_TO_EDGE );
